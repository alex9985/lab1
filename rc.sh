...
#akuzmenk -created forward chain filters
...
#
#akuzmenk -created input chain filters
...
#akuzmenk -created output chain filters
...
#akuzmenk -created POSTROUTING chain filters
$IPTABLES -t nat -A POSTROUTING -d 8.8.8.8/32 -o $WEB_IP_ADDRESS -p icmp -j MASQUERADE
$IPTABLES -t nat -A POSTROUTING -d 8.8.8.8/32 -o $WEB_IP_ADDRESS -p udp -j MASQUERADE
