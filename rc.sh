...
$IPTABLES -A FORWARD -i $Internet_IFACE -o $Client_NET_IFACE -m state --state RELATED,ESTABLISHED -j ACCEPT

$IPTABLES -A FORWARD -o $Client_NET_IFACE -i $Internet_IFACE -m state --state RELATED,ESTABLISHED -j ACCEPT

$IPTABLES -A FORWARD -i $Client_NET_IFACE -o $Internet_IFACE -p icmp -m conntrack --ctstate NEW -j ACCEPT

$IPTABLES -A FORWARD -p udp -j ACCEPT

$IPTABLES -A FORWARD -i $Client_NET_IFACE -o $Internet_IFACE -p icmp -j ACCEPT
...
# Alexander Kuzmenko - create accepting ESTABLISHED, RELATED

$IPTABLES -A INPUT -p tcp -m state --state RELATED,ESTABLISHED -j ACCEPT

$IPTABLES -A INPUT -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

# Alexander Kuzmenko - Disable ping on WEB SERVER WEB interface (30 Points)

$IPTABLES -A INPUT -p icmp --icmp-type 8 -d $Client_NET_IP -j DROP

## Alexander Kuzmenko - allow ping to server

$IPTABLES -A INPUT -p icmp -j ACCEPT

# Alexander Kuzmenko - Add Web localhost traffic (30 points)

$IPTABLES -A INPUT -p TCP --dport 80 -i $Client_NET_IFACE -d $WEB_IP_ADDRESS -j ACCEPT

# Alexander Kuzmenko - Allow Client access to Web Server (30 points)

$IPTABLES -A INPUT -d 10.0.2.1/32 -i $Client_NET_IFACE -p tcp -m tcp --dport 80 -j ACCEPT

$IPTABLES -A INPUT -i lo -j ACCEPT

# Alexander Kuzmenko - Adding DNS

$IPTABLES -A INPUT -i $Internet_IFACE -p udp -m udp --sport 53 -j ACCEPT

# Alexander Kuzmenko - adding logging

$IPTABLES -A INPUT -j LOG --log-prefix DROPPED-INGRESS-

# Alexander Kuzmenko - Drop all other traffic
...
##Alexander Kuzmenko - enable web traffic and extras (100 points)
...
$IPTABLES -A OUTPUT -o $Client_NET_IFACE -p tcp -m tcp --sport 80 -j ACCEPT

$IPTABLES -A OUTPUT -j LOG --log-prefix DROPPED-INGRESS-

$IPTABLES -A OUTPUT -o $WEB_IP_ADDRESS -p tcp -m tcp --dport 443 -j ACCEPT
...
# Alexander Kuzmenko - Enable ICMP forwarding for client and mask IP(30 points)

$IPTABLES -t nat -A POSTROUTING -s 10.0.2.4 -d 8.8.8.8 -o $Internet_IFACE -p ICMP -j MASQUERADE
