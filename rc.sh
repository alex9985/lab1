...
# Allow http traffic from client network to server networki
$IPTABLES -A FORWARD -s 10.0.2.4 -p ICMP -d 8.8.8.8 -j ACCEPT
$IPTABLES -A FORWARD -s 8.8.8.8 -p ICMP -d 10.0.2.4 -j ACCEPT
...
# Alexander Kuzmenko - allow syn
$IPTABLES -A INPUT -p TCP --syn -j ACCEPT
 
$IPTABLES -A INPUT -p icmp --icmp-type echo-reply -j ACCEPT
#$IPTABLES -A INPUT -s 10.0.2.4 -p icmp -d 8.8.8.8 -j ACCEPT
# Alexander Kuzmenko - Disable ping on WEB SERVER WEB interface (30 Points)
$IPTABLES -A INPUT -s 10.0.2.4/32 -p icmp --icmp-type 8 -d $Client_NET_IP -j DROP

# Alexander Kuzmenko - Allow Client access to Web Server (30 points)
$IPTABLES -A INPUT -s 10.0.2.4/32 -i $Client_NET_IFACE -d $NAT_WEB_IP_ADDRESS -p tcp -m tcp --dport 80 -j ACCEPT

# Alexander Kuzmenko - allow traffic on loopback interface
...
$IPTABLES -A OUTPUT -o $Client_NET_IFACE -d 10.0.2.4 -p tcp -m tcp --sport 80 -j ACCEPT
